...
const Timer = ({ initialTime }: TimerProps) => {
  ...
  useEffect(() => {
    ...
    const storeStartTime = localStorage.getItem("startTime");

    if (storeTimeLeft && storeStartTime) {
      const storedTimeLeftNum = parseInt(storeTimeLeft, 10);
      const storedStartTimeNum = parseInt(storeStartTime, 10);
